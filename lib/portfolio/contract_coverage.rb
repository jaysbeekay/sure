  ...
  class ContractCoverage
    ...
    def verify!
      ...
      rows.each do |id, documented_classes|
        ...
        manifest_classes = entries.map { |entry| entry.fetch("class") }.uniq.sort
        unless documented_classes.sort == manifest_classes
    ...
    private
      ...
      def verify_entry!(id, entry)
        ...
        fail!("#{id}: missing #{entry.fetch('file')}") unless file.file?

        source = file.read
        class_name = entry.fetch("class")
        fail!("#{id}: #{class_name} is not declared in #{entry.fetch('file')}") unless source.include?("class #{class_name} <")

        tests = Array(entry.fetch("tests"))
        fail!("#{id}: #{class_name} lists no tests") if tests.empty?
        tests.each do |name|
          next if source.include?(%(test "#{name}"))

          fail!("#{id}: missing test #{name.inspect} in #{entry.fetch('file')}")
        end
      end
