...
| P29 | Every hub series (`value_series`, `holdings_value_series`, `gains_series`) is trimmed to the first date all linked accounts in the scope have provider history for, the way the account charts are (`Balance::LinkedInvestmentSeriesNormalizer`), so the balance rows before a broker's first snapshot are not charted as a portfolio appearing from zero. Unlinked accounts impose no start. | `InvestmentStatementTest` "value_series does not chart leading zeros before a linked account's first supported history"; `Balance::LinkedInvestmentSeriesNormalizerTest` "trim_to_supported_history drops the points before the common supported start" | jaysbeekay/sure#120 finding 3 |
...
| P31 | The hub's holdings table (`holdings_table_rows(sort:, dir:)`) is the P1 roll-up with per-account positions. Cost basis and unrealised P&L come only from stored cost bases (locked, or positive, the rule `Holding#avg_cost` applies before it falls back to trades); a position without one leaves both blank and flags the row, so the table never issues a trades query per row. Sort keys are `value`, `weight`, `return`, `day_change`, `name` with `asc` / `desc`; anything else is `value desc`; rows without the sorted figure go last in both directions. | `InvestmentStatementTest` "holdings_table_rows rolls positions up per security with stored cost basis only", "holdings_table_rows sorts by a whitelisted key and puts rows without the figure last", "holdings_table_rows issues no query per holding" | jaysbeekay/sure#120 D4, finding 4 |
| P32 | `allocation_by` (account, currency or kind) weights sum to 100 within each grouping: accounts against their balances, currency and kind against the holdings plus each account's positive cash balance (negative cash is omitted). Kinds are `cash`, `crypto` (Binance MIC) and `standard`. Any other grouping is the P1 security roll-up with its cash row. | `InvestmentStatementTest` "allocation_by groups the portfolio by account, currency and kind with weights summing to 100" | jaysbeekay/sure#120 D5 |
| P33 | `data_quality_issues(as_of:)` lists: a holding with no stored cost basis and no buy trade to compute one from (`missing_cost_basis`); a security whose latest price is older than five days before `as_of`, or that has no price (`stale_price`); a security whose `provider_status` is not `:ok` (`provider`). Cash securities are never listed. Two bounded queries (buy-trade pairs, latest price dates). | `InvestmentStatementTest` "data_quality_issues flags missing cost basis, stale prices and unhealthy providers", "data_quality_issues issues a bounded number of queries" | jaysbeekay/sure#120 D6, M5 |
...
| P40 | `data_quality_issues(as_of:)` lists: a holding whose `Holding#avg_cost` is nil, nothing stored and nothing the trades can compute by the holdings tab's own rules (`missing_cost_basis`); a security whose latest price is older than five days before `as_of`, or that has no price (`stale_price`); a security whose `provider_status` is not `:ok` (`provider`). Cash securities are never listed. Two bounded queries (the P37 cost-basis preload, latest price dates). | `InvestmentStatementTest` "data_quality_issues flags missing cost basis, stale prices and unhealthy providers", "data_quality_issues issues a bounded number of queries" | jaysbeekay/sure#120 D6, M5 |
