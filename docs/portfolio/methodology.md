...
| P27 | A security whose current holdings sum to zero family-currency value (a position with no price yet, so every `amount` is 0) is omitted from `top_holdings` and `allocation` rather than listed at weight 0, and appears once it is worth something. Holdings with `qty` 0 are never current holdings. | `InvestmentStatementTest` "a security whose holdings carry no value is omitted from top_holdings and allocation" | #133 review |
