...
| R4 | **Annualisation applies only to periods of 365 days or more.** `annualized_twr` and `annualized_mwr` both return nil below that. Annualising a good week produces a number no one should be shown: it states a figure the assets never earned, which is why GIPS requires that a return for a period under a year is not annualised. The period figures (`twr`, `mwr`) are reported instead, never withheld for shortness. | `Portfolio::PerformanceTest#test_annualized_twr_is_nil_for_periods_under_a_year` |
